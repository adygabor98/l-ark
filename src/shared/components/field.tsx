...
import { TableFieldRenderer } from './form/table-field';
import { SignatureFieldRenderer } from './form/signature-field';
...
const Field = (props: IPropTypes) => {
    ...
    const { simple, small, disabled, name, type, hideInfinity, dataType, control, disableIds, suffix, required, label, placeholder, validators, pattern, className, multiple, mode, onSelectChange } = props;
    const { params } = props;
    ...
    const renderType = (field: any): ReactElement => {
        switch( type ) {
            ...
            case 'number':
                return <NumberField field={field} suffix={suffix} hideInfinity={hideInfinity} />;
            case 'radio':
                return <RadioField field={field} label={label} disabled={disabled} dataType={dataType as string} params={params as any} />;
            case 'image':
            ...
            case 'table':
                return <TableFieldRenderer field={field} columns={params} disabled={disabled} />
            case 'signature':
    ...
    return (
        <Controller
            ...
            render={({ field, fieldState }) =>
                type === 'toggle-switch' || type === 'table' ?
                    renderType(field)
                : simple && label ?
                    <div className='flex flex-col gap-2'>
                        <label className="text-[11px] font-[Lato-Bold] text-black/40 uppercase tracking-widest"> { label }{ required && <span className="text-red-500 ml-0.5">*</span> } </label>
                        <div className={`w-full flex flex-col border-[0.5px] ${fieldState.invalid ? 'h-auto border-red-400' : type === 'textarea' ? 'min-h-25 h-auto border-border/50' : type === 'checkbox' ? 'min-h-15 border-border/50' : 'h-auto border-border/50'} rounded-md ${className} bg-secondary/50 focus:bg-background focus:border-primary/30 transition-all shadow-sm`}>
                            { renderType(field) }
                        </div>
                ...
                :
                    <fieldset className={`w-full flex flex-col border-[0.5px] ${fieldState.invalid ? 'h-auto border-red-400' : type === 'textarea' ? 'min-h-25 h-auto border-border/50' : type === 'checkbox' ? 'min-h-15 border-border/50' : 'h-auto border-border/50'} rounded-md ${className} bg-secondary/50 focus:bg-background focus:border-primary/30 transition-all shadow-sm`}>
                        { label && <legend className={`h-fit w-fit ml-2 px-2 text-sm font-[Lato-Bold]! select-none ${fieldState.invalid ? 'text-red-600' : 'text-tertiary'}`}> { label }{ required && <span className="text-red-500 ml-0.5">*</span> } </legend> }
                        { renderType(field) }
                        { fieldState.invalid && <p className='text-red-400 text-[14px] pl-3 pb-1'> { fieldState.error?.message ? fieldState.error?.message : t('messages.required') } </p> }
                    </fieldset>
            }
