...
import { OperationBlueprintStatus, OperationInstanceStatus, OperationType, type OperationInstance } from "@l-ark/types";
...
const RequestGlobalModal = (props: PropTypes): ReactElement => {
	/** Retrieve component properties */
	const { instance, onClose, onSuccess } = props;
	/** Navigation utilities */
	const navigate = useNavigate();
	/** Operation api utilities */
	const { blueprints, instances, retrieveBlueprints, retrieveInstances, createInstance, linkInstances, updateInstanceStatus } = useOperation();
	/** Toast utilities */
	...
	// Get required other blueprints for the selected global via its prerequisites
	const requiredOtherBlueprints = useMemo(() => {
		if (!selectedGlobalBlueprint) return [];
		return ((selectedGlobalBlueprint as any).prerequisites ?? []).map((p: any) => p.requiredBlueprint).filter(Boolean);
	}, [selectedGlobalBlueprint]);

	// Set of required blueprint IDs (normalized to number) for quick lookup
	const requiredBlueprintIds = useMemo(
		() => new Set(requiredOtherBlueprints.map((bp: any) => Number(bp.id))),
		[requiredOtherBlueprints]
	);

	// Get eligible completed other instances for each required blueprint
	// FIX: normalize bp.id from GraphQL string to number for correct comparison with blueprintId (number)
	const eligibleInstances = useMemo(() => {
		...
		for (const reqBp of requiredOtherBlueprints) {
			...
			map[bpId] = instances.filter(
				(inst) =>
					inst.blueprintId === bpId &&
					inst.officeId === instance.officeId &&
					(inst.status === OperationInstanceStatus.COMPLETED_READY ||
						inst.status === OperationInstanceStatus.COMPLETED ||
						inst.id === instance.id)
			);
		}
		return map;
	...
	const additionalEligibleInstances = useMemo(() => {
		...
		return instances.filter(
			(inst) =>
				inst.id !== instance.id &&
				inst.officeId === instance.officeId &&
				(inst.blueprint as any)?.type === OperationType.OTHER &&
				!requiredBlueprintIds.has(inst.blueprintId) &&
				(inst.status === OperationInstanceStatus.COMPLETED_READY ||
					inst.status === OperationInstanceStatus.COMPLETED)
		);
	}, [selectedGlobalBlueprint, instances, instance.id, instance.officeId, requiredBlueprintIds]);

	const toggleOtherInstance = (instanceId: number) => {
		setSelectedOtherInstanceIds((prev) =>
			prev.includes(instanceId) ? prev.filter((id) => id !== instanceId) : [...prev, instanceId]
		);
	};

	// FIX: normalize bp.id to number; fall back to the instance prop when not in the retrieved list
	const allRequirementsMet = useMemo(() => {
		if (!selectedGlobalBlueprint) return false;
		if (requiredOtherBlueprints.length === 0) return true;

		return requiredOtherBlueprints.every((bp: any) => {
			const bpId = Number(bp.id);
			return selectedOtherInstanceIds.some((instId) => {
				const inst =
					instances.find((i) => i.id === instId) ??
					(instId === instance.id ? instance : undefined);
				return inst?.blueprintId === bpId;
			});
		});
	...
	const handleRequest = async () => {
		...
		const response = await createInstance({
			input: {
				...
				description: selectedGlobalBlueprint.description,
			},
		});

		// CREATE_INSTANCE returns ApiResponse (success/message/errors) — no id in the response.
		const createResult = (response as any)?.data?.data;
		if (!createResult?.success) return;

		// Close modal and notify immediately — don't block on linking/status updates
		onToast({ message: `Global operation "${selectedGlobalBlueprint.title}" requested successfully`, type: "success" });
		onSuccess?.();
		onClose();

		// Find the newly created global instance by matching blueprintId + officeId (use String to avoid number/string GraphQL type mismatch)
		...
		const newGlobalInstance = allInstances
			.filter((i: any) =>
				String(i.blueprintId) === String(selectedGlobalBlueprint.id) &&
				String(i.officeId) === String(instance.officeId)
			)
			.sort((a: any, b: any) => Number(b.id) - Number(a.id))[0];

		if (newGlobalInstance?.id) {
			// Link all selected OTHER instances to the new global
			await linkInstances({
				input: {
					sourceInstanceId: String(newGlobalInstance.id),
					targetInstanceIds: selectedOtherInstanceIds.map(String),
					linkType: "GLOBAL_OTHER",
				},
			});

			// Mark all linked OTHER instances as COMPLETED
			await Promise.all(
				selectedOtherInstanceIds.map((instId) =>
					updateInstanceStatus({ id: instId, status: OperationInstanceStatus.COMPLETED })
				)
			);
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center">
			...
			<div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col animate-in zoom-in-95 fade-in duration-300">
				...
				<div className="flex-1 overflow-y-auto p-6 space-y-6">
					{/* Step 1: Select Global Blueprint */}
					<div>
						...
						{availableGlobalBlueprints.length === 0 ? (
							...
							<div className="space-y-2">
								{availableGlobalBlueprints.map((bp) => (
									<button
										key={bp.id}
										onClick={() => setSelectedGlobalBlueprintId(bp.id)}
					...
					{/* Step 2: Link Required Instances */}
					{selectedGlobalBlueprint && requiredOtherBlueprints.length > 0 && (
						<div className="animate-in slide-in-from-bottom-4 fade-in duration-500">
							...
							<div className="space-y-4">
								{requiredOtherBlueprints.map((reqBp: any) => {
									const bpId = Number(reqBp.id);
									const eligible = eligibleInstances[bpId] ?? [];
												...
												<div className="space-y-1.5">
													{eligible.map((inst) => {
														...
														return (
															<button
																key={inst.id}
																onClick={() => !isCurrent && toggleOtherInstance(inst.id)}
																disabled={isCurrent}
																className={`w-full text-left p-3 rounded-lg border transition-all duration-200 ${
																	isCurrent
																		? "border-[#FFBF00]/40 bg-amber-50/40 cursor-default"
																		: isSelected
																	...
																	{(isSelected || isCurrent) && (
																		<div
																			className={`w-4 h-4 rounded-full flex items-center justify-center ${
																				isCurrent ? "bg-[#FFBF00]" : "bg-emerald-500"
																			}`}
																		>
					...
					{/* Step 3: Additional Optional Links */}
...
export default RequestGlobalModal;
