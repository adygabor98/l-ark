...
import type { ExportRow, ExportPageConfig } from "../../../templates/export-layout/export-layout.models";
import { PAGE_DIMS, PX_PER_MM, ROW_GAP_PX } from "../../../templates/export-layout/export-layout.constants";
...
export const useExportPagination = ({ rows, pageConfig, loading }: UseExportPaginationOptions) => {
	...
	// Pagination
	const pages = useMemo(() => {
		if (!pageConfig || rows.length === 0) return [{ rows: [] as ExportRow[], yOffset: 0 }];
