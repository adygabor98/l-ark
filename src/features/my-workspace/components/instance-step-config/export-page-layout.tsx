...
export const Watermark = ({ text, pageWidth, color, opacity }: { text: string; pageWidth: number; color?: string; opacity?: number }): ReactElement | null => {
	...
	return (
		<div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none overflow-hidden" style={{ zIndex: 0 }}>
			<span className="font-[Lato-Black] uppercase tracking-widest text-center"
...
export const HeaderBand = ({ pageConfig, zoneExtensions }: { pageConfig: ExportPageConfig; zoneExtensions: any[] }): ReactElement | null => {
	...
	if ( pageConfig.headerContent ) {
		try {
			headerHtml = generateHTML(pageConfig.headerContent as never, zoneExtensions);
		} catch ( e: any ) {
			console.error(e);
		}
	}
...
export const FooterBand = ({ pageConfig, pageNum, totalPages, zoneExtensions }: { pageConfig: ExportPageConfig; pageNum: number; totalPages: number; zoneExtensions: any[]; }): ReactElement | null => {
	if ( !pageConfig.showFooter )
		return null;
	...
	if ( pageConfig.footerContent ) {
		try {
			footerHtml = generateHTML(pageConfig.footerContent as never, zoneExtensions);
		} catch ( e: any ) {
			console.error(e);
		}
	}
