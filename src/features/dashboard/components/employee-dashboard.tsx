...
import { Activity, Calendar, Bell, Clock, ChevronRight } from "lucide-react";
...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
    ...
    COMPLETED_READY:  { label: "Ready to Close",   color: "bg-amber-100 text-amber-600" },
    LINKED:           { label: "Linked",           color: "bg-purple-100 text-purple-600" },
    CLOSED:           { label: "Closed",           color: "bg-green-100 text-green-600" },
    PENDING_PAYMENT:  { label: "Pending Payment",  color: "bg-orange-100 text-orange-600" },
    PARTIALLY_CLOSED: { label: "Partially Closed", color: "bg-emerald-100 text-emerald-600" },
};
...
export const EmployeeDashboard = (): ReactElement => {
    ...
    useEffect(() => {
        if (user?.id) retrieveInstances({ assignedToId: String(user.id) });
        retrieveAppointments({ type: "WEEK" });
        retrieveNotifications({ unreadOnly: true, limit: 5 });
    }, [user?.id]);

    const activeInstances = useMemo(
        () => instances.filter(i => i.status !== "CLOSED"),
        [instances]
    );

    const pendingStepsCount = useMemo(
        () => instances.reduce((total, inst) =>
            total + (inst.stepInstances?.filter(s => s.status === "PENDING" || s.status === "IN_PROGRESS").length ?? 0), 0),
        [instances]
    );

    const todayStr = format(new Date(), "yyyy-MM-dd");
    const todayAppointments = useMemo(
        () => appointments.filter(a => format(new Date(a.startAt), "yyyy-MM-dd") === todayStr),
        ...
        <div className="space-y-8">
            ...
            <div>
                <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-[#1A1A1A]">
                    Good morning, <span className="text-[#FFBF00]">{user?.firstName}</span>
                </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <GlassCard className="p-6 bg-gradient-to-br from-[#1A1A1A] to-[#2d2d2d] text-white border-none">
                    ...
                    <div className="text-xs opacity-60 uppercase tracking-wider mt-1">Active Operations</div>
                </GlassCard>
                <GlassCard className="p-6">
                    ...
                    <div className="text-xs text-muted-foreground uppercase tracking-wider mt-1">Pending Steps</div>
                </GlassCard>
                <GlassCard className="p-6">
                    ...
                    <div className="text-xs text-muted-foreground uppercase tracking-wider mt-1">Today's Meetings</div>
                </GlassCard>
            </div>
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    <GlassCard className="p-0">
                        <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
                            ...
                            <span className="text-xs text-muted-foreground">{activeInstances.length} active</span>
                        </div>
                        <div className="divide-y divide-gray-50">
                            {activeInstances.length === 0 ? (
                                <div className="p-8 text-center text-sm text-muted-foreground">No active operations</div>
                                ...
                                const statusInfo = STATUS_LABELS[instance.status] ?? { label: instance.status, color: "bg-gray-100 text-gray-600" };
                                return (
                    ...
                    <GlassCard className="p-6">
                        ...
                        {todayAppointments.length === 0 ? (
                            <div className="py-4 text-center text-sm text-muted-foreground">No appointments today</div>
                        ) : (
