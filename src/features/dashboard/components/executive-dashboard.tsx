...
import {
    Activity, Users, Building, Bell, Calendar, Briefcase,
...
import { useOffice } from "../../../server/hooks/useOffice";
...
const StatCard = ({ title, value, trend, icon: Icon, dark = false }: {
    title: string; value: string | number; trend?: number; icon: React.ElementType; dark?: boolean;
}) => (
...
// Pipeline stages shown in the flow widget (logical order)
...
export const ExecutiveDashboard = (): ReactElement => {
    ...
    // --- Pipeline counts (status distribution) ---
    ...
    // --- Top Offices by total operations ---
    const topOffices = useMemo(() => {
        ...
        instances.forEach(inst => {
            ...
            if (!officeOpCounts[key]) officeOpCounts[key] = { name: inst.office.name, total: 0, closed: 0 };
            officeOpCounts[key].total++;
            if (inst.status === "CLOSED" || inst.status === "PARTIALLY_CLOSED") officeOpCounts[key].closed++;
        });
        return Object.values(officeOpCounts)
    ...
    // --- Employee performance (top contributors) ---
    const employeePerformance = useMemo(() => {
        ...
        instances.forEach(inst => {
            ...
            if (!perf[key]) perf[key] = { name, avatar, opened: 0, closed: 0 };
            perf[key].opened++;
            if (inst.status === "CLOSED" || inst.status === "PARTIALLY_CLOSED") perf[key].closed++;
        });
    ...
    return (
        <div className="space-y-8">
            ...
            <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">{today} · {user?.firstName} {user?.lastName}</p>
            </div>
                ...
                <div className="p-6 md:p-8">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4 relative min-w-0">
                        ...
                        {PIPELINE_ORDER.map(status => {
                            ...
                            const hasAlert = status === "COMPLETED_READY" && count > 0;
                            return (
                            ...
                            {topOffices.map((office, idx) => (
                                <div key={idx} className="group cursor-pointer">
                                    ...
                                    <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden">
                                        <div
                                            className={`h-full rounded-full transition-all duration-500 ${idx === 0 ? "bg-[#FFBF00]" : "bg-[#1A1A1A]"}`}
                            ...
                            {employeePerformance.map((emp, i) => (
                                <div key={i} className="flex items-center justify-between group">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-sm font-semibold text-[#1A1A1A] group-hover:text-[#FFBF00] transition-colors truncate max-w-[100px]">
                                            {emp.name}
                                        </span>
                                    </div>
                                    <div className="flex gap-4">
